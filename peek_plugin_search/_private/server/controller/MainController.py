...
from peek_plugin_search._private.tuples.AddIntValueActionTuple import \
    AddIntValueActionTuple
from peek_plugin_search._private.tuples.StringCapToggleActionTuple import \
...
class MainController(TupleActionProcessorDelegateABC):
    ...
    def _processCapToggleString(self, action: StringCapToggleActionTuple):
        try:
            ...
            row = (session.query(SearchPropertyTuple)
                   .filter(SearchPropertyTuple.id == action.propertyId)
                   .one())

            # Exit early if the string is empty
            if not row.string1:
                logger.debug("string1 for SearchPropertyTuple.id=%s is empty")
                return
