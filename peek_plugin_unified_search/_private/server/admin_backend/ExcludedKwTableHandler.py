...
filtKey = {"key": "admin.Edit.ExcludedKwTuple"}
filtKey.update(unifiedSearchFilt)
...
def makeExcludedKwTableHandler(tupleObservable, dbSessionCreator):
    handler = __CrudHandler(dbSessionCreator, ExcludedKwTuple,
                            filtKey, retreiveAll=True)

    logger.debug("Started")
    handler.addExtension(ExcludedKwTuple, __ExtUpdateObservable(tupleObservable))
    return handler
